...
def main(args):
    ...
    unet_lora_parameters1 = []
    unet_lora_parameters2 = []

    for attn_processor_name, attn_processor in unet.attn_processors.items():
        # Parse the attention module.
        # if "attn2" not in attn_processor_name:
        #     continue
        attn_module = unet
        ...
        unet_lora_parameters1.extend(attn_module.to_q.lora_layer.down.parameters())
        unet_lora_parameters1.extend(attn_module.to_k.lora_layer.down.parameters())
        unet_lora_parameters1.extend(attn_module.to_v.lora_layer.down.parameters())
        unet_lora_parameters1.extend(attn_module.to_q.lora_layer.up.parameters())
        unet_lora_parameters1.extend(attn_module.to_k.lora_layer.up.parameters())
        unet_lora_parameters1.extend(attn_module.to_v.lora_layer.up.parameters())
        unet_lora_parameters2.extend(attn_module.to_q.lora_layer.diagnal.parameters())
        unet_lora_parameters2.extend(attn_module.to_k.lora_layer.diagnal.parameters())
        unet_lora_parameters2.extend(attn_module.to_v.lora_layer.diagnal.parameters())
        unet_lora_parameters1.extend(attn_module.to_out[0].lora_layer.down.parameters())
        unet_lora_parameters1.extend(attn_module.to_out[0].lora_layer.up.parameters())
        unet_lora_parameters2.extend(attn_module.to_out[0].lora_layer.diagnal.parameters())
    ...
    unet_lora_parameters_with_lr1 = {
        "params": unet_lora_parameters1,
        "lr": args.learning_rate,
    }

    unet_lora_parameters_with_lr2 = {
        "params": unet_lora_parameters2,
        "lr": args.learning_rate * args.lr_theta, # beilv
    }
    ...
    if args.train_text_encoder:
        ...
        params_to_optimize = [unet_lora_parameters_with_lr1, unet_lora_parameters_with_lr2]
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            # 2) optimizer を C 専用で定義し直す
            optimizer = optimizer_class(
                [
                    {
                        "params": c_params,
                        "lr": args.learning_rate * args.lr_theta,  # C用のlr
                    }
                ],
                betas=(args.adam_beta1, args.adam_beta2),
                weight_decay=args.adam_weight_decay,
            ...
            # 4) mixed_precision=fp16 のときは scaler もリセット
            if accelerator.mixed_precision == "fp16":
                from torch.amp import GradScaler
                accelerator.scaler = GradScaler("cuda", enabled=True)

            # 5) 新しく作った optimizer / scheduler を prepare に通す
    ...
    for p in unet_lora_parameters1:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                styles = batch["style_index"]
                prompt_embeds, unet_add_text_embeds = compute_text_embeddings(
                    prompts, text_encoders, tokenizers
                )
                if global_step<= initial_global_step+3:
                    print(batch["path"])

                model_input = vae.encode(pixel_values,None).latent_dist.sample()
                model_input = model_input * vae.config.scaling_factor
                
                t1 = torch.tensor([1], device=accelerator.device, dtype=weight_dtype)
                ...
                if not args.train_text_encoder:
                    ...
                    model_pred = unet(noisy_model_input,timesteps,t1,prompt_embeds_input,added_cond_kwargs=unet_added_conditions,).sample
                ...
                #     for name, param in unet.named_parameters():
                #         if "diagnal.omegas_weights" in name:
                #             c_before[name] = param.detach().clone()
                # if global_step == initial_global_step + 10 and accelerator.is_main_process:
                #     for name, param in unet.named_parameters():
                #         if "diagnal.omegas_weights" in name:
                #             delta = (param.detach() - c_before[name]).abs().mean().item()
                #             print("delta diagnal", name, "=", delta)
                #             cnt += 1
                #             if cnt >= 10:
                #                 break

                accelerator.backward(loss) 
                # if global_step == initial_global_step and accelerator.is_main_process:
                #     print("=== diagnal in optimizer check (first 10) ===")
                #     cnt = 0
                #     for name, param in unet.named_parameters():
                #         if "diagnal.omegas_weights" in name:
                #             in_optim = any(
                ...
                #             )
                #             print(name, "in optimizer =", in_optim)
                #             cnt += 1
                #             if cnt >= 10:
                #                 cnt = 0
                #                 break                    
                #     print("=== diagnal grad check (first 10) ===")
                #     cnt = 0
                #     for name, param in unet.named_parameters():
                #         if "diagnal.omegas_weights" in name:
                #             grad_mean = None
                ...
                #                 "grad is None =", param.grad is None,
                #                 "grad_mean =", grad_mean,
                #             )
                #             cnt += 1
                #             if cnt >= 10:
            ...
            # if (
            #     (num_update_steps_per_epoch*epoch+step)%args.validation_epochs==0
            # ):
            ...
            #     )
            #     time_t = torch.tensor([0], device=accelerator.device, dtype=weight_dtype)
            #     print("validation: ", time_t)

            #     unet_lora_layers = unet_lora_state_dict(accelerator.unwrap_model(unet).to(torch.float32))

            #     # for k, v in unet_lora_layers.items():
            #     #     if k.endswith("down.weight"):
            #     #         a1 = unet_lora_layers[k]
            #     #         b1 = unet_lora_layers[k.replace("down.weight", "up.weight")]
            #     #         omegas_weights = unet_lora_layers[k.replace("down.weight", "diagnal.omegas_weights")]
            #     #         weights = unet_lora_layers[k.replace("down.weight", "diagnal.weights")]
            #     #         cos_v = torch.sum(weights * torch.cos(time_t.reshape(2, 1, 1) * omegas_weights), dim=0)
            #     #         sin_v = torch.sum(weights * torch.sin(time_t.reshape(2, 1, 1) * omegas_weights), dim=0)
            #     #         mask = torch.eye(args.rank, device=a1.device, dtype=a1.dtype)
            ...
            #     #         eval(f'pipeline.unet.{target_layer}').weight.data += dW.to(pipeline.device)
            ...
            #             omegas_weights = unet_lora_layers[
            #                 k.replace("down.weight", "diagnal.omegas_weights")
            #             ]  
            #             t = time_t
            #             I = torch.eye(omegas_weights.size(0), device=a1.device, dtype=a1.dtype)
            #             matrix = I + t * omegas_weights  
            #             dW = b1 @ (matrix.T) @ a1  
            #             target_layer = target_layer_from_sd_name(k)
            #             eval(f"pipeline.unet.{target_layer}").weight.data += dW.to(pipeline.device)
            ...
            #     images = [
            #         pipeline(**pipeline_args, generator=torch.Generator(device=accelerator.device).manual_seed(_)).images[0]
            ...
            # === CSV に追記（メインプロセスのみ） ===
            # if accelerator.is_main_process:
            #     # epoch・step は任意の記録。step はループ内の局所ステップを入れておく例
            #     step_in_epoch = step
            #     with open(csv_path, mode="a", newline="", encoding="utf-8") as f:
            #         writer = csv.writer(f)
            #         writer.writerow([global_step, logs["loss"]])
